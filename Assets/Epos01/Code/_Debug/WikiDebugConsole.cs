...
public class WikiDebugConsole : MonoBehaviour
{
    ...
    public static bool isVisible
    {
        get
        {
            return WikiDebugConsole.instance.visible;
        }

        set
        {
            WikiDebugConsole.instance.visible = value;
            if (value == true)
            {
                WikiDebugConsole.instance.Display();
            }
            else if (value == false)
            {
                WikiDebugConsole.instance.ClearScreen();
            }
        }
    }
    ...
    public static bool isDraggable
    {
        get
        {
            return WikiDebugConsole.instance.draggable;
        }

        set
        {
            WikiDebugConsole.instance.draggable = value;

        }
    }


    private static WikiDebugConsole s_Instance = null;   // Our instance to allow this script to be called without a direct connection.
    public static WikiDebugConsole instance
    {
        get
        {
            if (s_Instance == null)
            {
                s_Instance = FindObjectOfType(typeof(WikiDebugConsole)) as WikiDebugConsole;
                if (s_Instance == null)
                {
                    GameObject console = new GameObject();
                    console.AddComponent<WikiDebugConsole>();
                    console.name = "DebugConsoleController";
                    s_Instance = FindObjectOfType(typeof(WikiDebugConsole)) as WikiDebugConsole;
                    WikiDebugConsole.instance.InitGuis();
                }
    ...
    void Awake()
    {
        ...
        WikiDebugConsole.Log("WikiDebugConsole running...");
    ...
    {
        WikiDebugConsole.instance.AddMessage(message, color);

    }
    //++++ OVERLOAD ++++
    public static void Log(string message)
    {
        WikiDebugConsole.instance.AddMessage(message);
    }

    public static void Clear()
    {
        WikiDebugConsole.instance.ClearMessages();
    }
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
